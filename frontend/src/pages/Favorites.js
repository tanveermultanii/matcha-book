...
function Favorites() {
  ...
  return (
    <div className="min-h-screen bg-matcha-light dark:bg-night-library text-matcha-dark dark:text-night-text p-6">
      <h2 className="text-3xl font-bold mb-4 text-center">Your Favorite Books</h2>
      {favorites.length === 0 ? (
        <p className="mb-6 text-center">
          No favorite books yet. Go add some from the search or recommendations page!
        </p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {favorites.map((book) => (
            <div
              key={book.id}
              className="bg-white dark:bg-night-library-light p-4 rounded-lg shadow-md relative"
            >
              <img
